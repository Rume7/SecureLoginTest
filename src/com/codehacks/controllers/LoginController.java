package com.codehacks.controllers;
...
import com.codehacks.helpers.UserHelper;
import com.codehacks.pojo.User;
import com.codehacks.utilities.PasswordService;
...
public class LoginController extends HttpServlet {
	...
	/**
	 *  Processes GET request/response)
	 */
	...
	/**
	 *  Process POST requests/responses (logins)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (loginAttempts > 3) {
			...
			// encrypt the password to check against what's stored in the DB
			...
			if (user != null) {
				// invalidate current sessions, then get new session for our user (combats: session hijacking)
				session.invalidate();
			...
			// user doesn't exist, redirect to previous page and show error
				...
				// track login attempts (combats: brute force)
